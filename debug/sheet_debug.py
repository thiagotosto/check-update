...
class Sheet():
	...
	def search(self, value, column):											#procura o Valor SH
		for row_index in range(self.sheet.max_row):
			#print "Debugando!", row_index, " Valor buscado:", value, " Valor atual: ", self.sheet.cell(row=row_index+1 , column=column+1).value 
...
'''
teste = Sheet('./files/Controle de servidores VM_NEW Versao atualizada.xlsx')
teste.load_sheet('inventario rio e sp')
teste.load_columns(4)
print "TESTANDO DICIONÁRIO", teste.columns['SERIAL'], "\n\n\n"
hold = teste.search('BRC931N036', teste.columns['SERIAL'])
print hold
teste.add_element((teste.columns['SERIAL'],teste.columns['MODELO']), (2222, 1111))
teste.add_element((teste.columns['SERIAL'],teste.columns['MODELO']), (3333, 1111))
teste.save('./files/teste_sheet.xlsx')
'''
